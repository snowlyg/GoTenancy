...
### Initialize [Render](https://GoTenancy/libs/render)

```go
import "GoTenancy/libs/render"
...
1. The template name. In this example [Render](https://GoTenancy/libs/render) will look up template `index.tmpl` from view paths. the default view path is `{current_repo_path}/app/views`, and you could register more view paths.
...
[Render](https://GoTenancy/libs/render) will find the layout at `{current_repo_path}/app/views/layouts/new_layout.tmpl`.

### Render with helper functions

Sometimes you may want to have some helper functions in your template. [Render](https://GoTenancy/libs/render) supports passing helper functions by `Funcs` function.
...
Put the [Render](https://GoTenancy/libs/render) inside [Responder](./responder.md) handle function like this.

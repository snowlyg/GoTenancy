...
func (admin *Admin) SetAssetFS(assetFS assetfs.Interface) {
	...
	_ = admin.AssetFS.RegisterPath(filepath.Join(utils.AppRoot, "app/views/qor"))
	admin.RegisterViewPath("libs/admin/views")
...
func (admin *Admin) RegisterViewPath(pth string) {
	if admin.AssetFS.RegisterPath(filepath.Join(utils.AppRoot,  pth)) != nil {

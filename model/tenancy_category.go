...
type TenancyCategory struct {
	...
	Sort     int32  `gorm:"index:sort;column:sort;type:mediumint;not null" json:"sort"`      // 排序
	Pic      string `gorm:"column:pic;type:varchar(128);not null;default:''" json:"pic"`     // 图标
	IsShow   bool   `gorm:"column:is_show;type:tinyint(1);not null;default:1" json:"isShow"` // 是否显示
	Level    uint   `gorm:"column:level;type:int unsigned;not null;default:0" json:"level"`  // 等级

	TenancyID uint `gorm:"column:tenancy_id;type:int unsigned;default:0" json:"tenancyId"` // 商户id
}

...
func CreateConfig(ctx iris.Context) {
	...
	_ = ctx.ReadJSON(&config)
	if err := utils.Verify(config, utils.CreateConfigVerify); err != nil {
...
func UpdateConfig(ctx iris.Context) {
	var config model.SysConfig
	_ = ctx.ReadJSON(&config)
	if err := utils.Verify(config, utils.UpdateConfigVerify); err != nil {
		response.FailWithMessage(err.Error(), ctx)
	}
...
func GetConfigByName(ctx iris.Context) {
	...
	_ = ctx.ReadJSON(&req)
	if err := utils.Verify(req, utils.GetConfigVerify); err != nil {
		response.FailWithMessage(err.Error(), ctx)
		return
	}

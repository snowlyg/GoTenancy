...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if g.TENANCY_CONFIG.Mysql.LogZap != "" {
		switch len(data) {
		case 0:
			g.TENANCY_LOG.Info(message)
		case 1:
			g.TENANCY_LOG.Info("gorm", zap.Any("src", data[0]))
		case 2:
			g.TENANCY_LOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			g.TENANCY_LOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
			g.TENANCY_LOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
		}
		return
	}
	switch len(data) {

...
func GetProfile(ctx iris.Context) {
	...
	user := tenant.NewRabcUser(userId, "")
	user.GetRabcUserById()
...
func CreateUser(ctx iris.Context) {
	...
	aul := new(homevalidates.CreateUpdateRabcUserRequest)
...
func GetAllUsers(ctx iris.Context) {
	...
	users := tenant.GetAllRabcUsers(name, orderBy, offset, limit)

	ctx.StatusCode(iris.StatusOK)
	_, _ = ctx.JSON(ApiResource(true, usersTransform(users), "操作成功"))
}

func usersTransform(users []*tenant.RabcUser) []*hometransformer.User {
	var us []*hometransformer.User
...
func userTransform(user *tenant.RabcUser) *hometransformer.User {
	u := &hometransformer.User{}
	g := gf.NewTransform(u, user, time.RFC3339)
	_ = g.Transformer()

	roleIds := utils.GetRolesForUser(user.ID)
	var ris []int
	var roleName string
	for num, roleId := range roleIds {
		ri, _ := strconv.Atoi(roleId)
		ris = append(ris, ri)
		role := tenant.NewRabcRole(uint(ri), "")
